...
def keep_bounds(population: np.ndarray,
                bounds: np.ndarray, corr_method: str) -> np.ndarray:
    """
    ...
    """
#     corr_method = 'saturate'
#     if corr_method == 'saturate':
#         np.clip(population, bounds[:,1], bounds[:,0])
#     else:
    return np.array([correct_bounds(x, bounds[:,1], bounds[:,0], corr_method)[0] for x in population])


def init_population(population_size: int, individual_size: int,
                    bounds: Union[np.ndarray, list], 
                    corr_method: str) -> np.ndarray:
    """
    ...
    population = np.random.uniform(-5, 5, (population_size, individual_size))
    if _write_output:
        print(f"@{len(population)}@")
        print(f"|{np.mean(np.std(population, axis=0))}|")
    return keep_bounds(population, bounds, corr_method)
...
def binary_mutation(population: np.ndarray,
    ...
    return keep_bounds(mutated, bounds, corr_method)
